...
-   **使用授权key进行初始化** 
    ...
    **说明：** 如果设备目录/sdcard/verify/verifysdk/ab.bin存在而且有效，可以使用`token=””`直接进行本地断网初始化。如果本地初始化失败，需要联网在线初始化，激活使用的授权key，从实人认证服务端CreateAuthKey接口获得，在线联网初始化会将最新的授权文件ab.bin下载到本地，完成SDK授权。
